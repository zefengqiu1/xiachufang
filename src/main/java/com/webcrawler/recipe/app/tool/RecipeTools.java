...
public class RecipeTools {
    ...
    @Tool("在本地菜谱库中查找最匹配的一道菜。输入用户原始问题或菜名，返回匹配分数、菜名和整理后的菜谱内容。")
    public String searchLocalRecipe(String query) {
        RecipeSearchHit hit = recipeSearchService.searchBest(query);
        if (hit == null) {
            return "未在本地菜谱库中找到可用结果。";
        }
        return """
                confidence: %.3f
                score: %.3f
                ...
                recipeRenderService.render(hit, query)
        ).trim();
    }
